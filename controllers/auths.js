...
exports.logins=  async(req,res)=>{
    try {
        ...
        if(!email || !password){
            return res.status(400).render('logins',{
        ...
        db.query('SELECT * FROM seller WHERE email = ?',[email], async(error,results)=>{
            if(!results || !(await bcrypt.compare(password,results[0].password))){
                ...
                res.status(200).redirect('/indexs');
            }
...
exports.registers = (req, res) => {
    ...
    db.query('SELECT email FROM seller WHERE email= ?', [email], async(error, results) => {
        ...
        if(!email || !password || !name || !passwordConfirm){
            return res.status(400).render('registers',{
                message: 'please provide an e-mail or password'
            })
        } 
        ...
        db.query('INSERT INTO seller SET ?', { name: name, email: email, password: hasedPassword }, (error, results) => {
            if (error) {
                console.log(error);
            } else {
                console.log(results);
